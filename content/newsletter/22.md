...
In the world of Rust programming, there's always something new and exciting to explore. Today, we're introducing a fascinating pet project (pun intended) that's been buzzing in the Rust community: [the Rust Ants Colony Simulation](https://github.com/bones-ai/rust-ants-colony-simulation).
 
This project, is an ant colony simulation implemented in Rust. Despite its current inefficiency, which allows it to handle only about 500 ants on an older laptop, it's a captivating exploration of Rust's capabilities. The simulation is built using Rust and the [Bevy game engine](https://bevyengine.org/), showcasing the versatility of Rust in different domains.

You can find the [timelapse video on Youtube](https://youtu.be/98pUSZAM_7M). 
...
In the realm of coding, safety is paramount, especially when it comes to applications like autonomous driving systems, medical equipment, or industrial control systems. Enter [Ferrocene](https://ferrous-systems.com/ferrocene/), a unique variant of the Rust compiler, designed with a singular purpose - to ensure the highest level of safety in electrical and electronic systems.
...
Join Ferrous Systems announcement party on [October 4th, 2023, online](https://www.eventbrite.com/e/a-decade-of-rust-with-ferrous-systems-tickets-680492891557?aff=oddtdtcreator).
...
For more information, check out the [Rayon GitHub page](https://github.com/rayon-rs/rayon). For a Youtube explainer checkout this [video from Let's Gets Rusty](https://youtu.be/YxG7PhZ3fb4).
